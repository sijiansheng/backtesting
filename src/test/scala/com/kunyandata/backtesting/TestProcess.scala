...
object TestProcess extends App {
  ...
  val producerHandler = KafkaProducerHandler(kafkaMap.get("brokerList").get, kafkaMap.get("sendTopic").get)
    ...
    val conditionGroupTwo = List(
      "2:涨跌幅大于0.08%小于0.09%",
      "2:跌幅大于0.06%小于0.07%",
      "2:振幅大于0.08%小于0.09%", "2:成交量大于60万小于80万",
      "2:股价大于11元小于12元", "2:收益率大于-0.07%小于-0.06%"
    )
    val resultsGroupTwo = List("002347", "300441", "002044", "300466", "600339", "300441")
